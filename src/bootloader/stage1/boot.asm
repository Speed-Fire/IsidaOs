...
start:
	...
	; some BIOSes might start us at 7C00:0000 instead of 0000:7C00. Make sure to start at 0000:07C0
	push es
	push word .after
	retf
...
.after:
	...
	; read drive parameters (sectors per head and head count)
	...
	and cl, 0x3f								; removing top 2 bits
	xor ch, ch
	...
	add ax, [bdb_reserved_sectors]			; ax = ax + reserved = LBA of root directory
	push ax
...
.search_kernel:
	...
	inc bx
	cmp bx, [bdb_dir_entries_count]			; check if it wasn't the last entry
	jl .search_kernel						; loop
